...
public:
    ...
    void remove_vertex(Vertex* vertex) {
        ...
        for(auto &hyperedge : hyperedges) {
            hyperedge->vertices.erase(std::remove(hyperedge->vertices.begin(), hyperedge->vertices.end(), vertex), hyperedge->vertices.end());
        }
    }
    
    void add_hyperedge(Hyperedge* hyperedge) {
        hyperedges.push_back(hyperedge);
    }

    void remove_hyperedge(Hyperedge* hyperedge) {
        hyperedges.erase(std::remove(hyperedges.begin(), hyperedges.end(), hyperedge), hyperedges.end());
    }

    int degree(Vertex* vertex) {
        int count = 0;
        for(auto &hyperedge : hyperedges) {
            if(std::find(hyperedge->vertices.begin(), hyperedge->vertices.end(), vertex) != hyperedge->vertices.end()) {
                count++;
            }
        }
        return count;
        ...
        for(auto &vertex : hyperedge->vertices) {
            if(std::find(visited.begin(), visited.end(), vertex) == visited.end()) {
                ...
                for(auto &he : hyperedges) {
                    if(std::find(he->vertices.begin(), he->vertices.end(), vertex) != he->vertices.end()) {
                        dfs_h(he, visited);
                    }
                }
            }
    ...
    Hypergraph sub_hypergraph(const std::vector<Vertex*> &vertices_to_remove) {
        ...
        for(auto &vertex : vertices_to_remove)
            sub_hypergraph.remove_vertex(vertex);
...
int main() {
    ...
    std::cout << "Vertices: ";
    for(auto &v : hypergraph.vertices)
        std::cout << v->id << " ";
    std::cout << "\nHyperedges: ";
